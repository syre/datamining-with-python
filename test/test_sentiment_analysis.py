...
class SentimentAnalysisTestCase(TestCase):
    ...
    def setUp(self):
        self.sa = sentiment_analysis.SentimentAnalysis("data/classifier.pickle")
    ...
    @mock.patch("sentiment_analysis.SentimentAnalysis._compare_comments_number")
    ...
    def test_classify_comments(self, comments_compare):
        ...
        video_sentiment, comments_sentiment = self.sa.classify_comments(comments)
    ...
    def test_eval(self):
        test_ratios_pos = [0.1, 0.25, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7, 0.85, 0.96]
        test_objects = []
        for ratio in test_ratios_pos:
            test_objects.append(models.VideoSentiment(id="dQw4w9WgXcQ", n_pos=ratio, n_neg=(1-ratio), result=""))

        result = []
        for object in test_objects:
            result.append(self.sa._eval(object))

        assert [res for res in result] == ["strong negative", "slight negative", "slight negative", "neutral",
                                           "neutral", "neutral", "slight positive", "slight positive",
